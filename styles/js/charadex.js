...
charadex.initialize.page = async (dataArr, config, dataCallback, listCallback, customPageUrl = false) => {
  ...
  for (let entry of charadexData) {
    ...
    if (entry.rarity2) entry.rarity2badge = `<span class="badge badge-${charadex.tools.scrub(entry.rarity2)}">${entry.traitcount} - ${entry.rarity2}</span>`; // Adds a rarity badge for the tendency traits section.
